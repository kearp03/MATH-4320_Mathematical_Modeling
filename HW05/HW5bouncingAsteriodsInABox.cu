...
void idle()
{
	// If we are not paused, then run the simulation
	if(Pause == 0) nBody();
}
...
// Function to handle keyboard presses during simulation
...
void setInitailConditions()
{
	...
	MassUnitConverter = 3.383 * pow(10, 20); // kg
	LengthUnitConverter = 940.0; // km
	TimeUnitConverter = 3640.0/3600.0; // hr
	...
	SphereDiameter = 1.0; // The diameter of an asteriod is 1.0 of our units, as we decided
	sphereRadius = SphereDiameter/2.0; // Radius is half diameter regardless of units
	SphereMass = 1.0; // The mass of an asteriod is 1.0 of our mass units, as we decided
	BoxSideLength = 5.0; // The box is 5.0 of our length units 
	MaxVelocity = 10.0; // The max velocity is 10.0 LengthUnit/TimeUnit
	halfBoxSideLength = BoxSideLength/2.0; // Half the length of the box does not depend on the units used
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		randomNumber = ((float)rand()/(float)RAND_MAX)*((175.0-150.0)/255.0) + 150.0/255.0;
		Color[i].x = randomNumber;
		// Mid values of green
		randomNumber = ((float)rand()/(float)RAND_MAX)*((100.0-50.0)/255.0) + 50.0/255.0;
		Color[i].y = randomNumber;
		// No blue
		randomNumber = ((float)rand()/(float)RAND_MAX)*(0);
	...
	TotalRunTime = 10*24/TimeUnitConverter;
	RunTime = 0.0;
	Dt = 0.001;
}
...
{	
	...
	printf("\n Time = %f hours", RunTime*TimeUnitConverter);
	RunTime += Dt;
