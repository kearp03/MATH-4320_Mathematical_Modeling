...
void getForces()
{
	...
	float4 d, unit, dv;
	float magnitude;
	float intersectionArea, bodyVolume; 
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				bodyVolume = (PI/24.0)*(2.0*SphereDiameter*SphereDiameter*SphereDiameter + 3.0*SphereDiameter*SphereDiameter*d.w -d.w*d.w*d.w);
				...
				magnitude = cSphere*6*intersectionArea/bodyVolume; // Calculate the magnitude as if inOut is negative, so the sphere are converging.
				if(0.0 < inOut) magnitude *= cSphereReduction; // If inOut is positive the sphere are diverging, apply the reduction.
